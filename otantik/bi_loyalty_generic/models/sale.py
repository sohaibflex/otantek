...
class AccountMoveInherit(models.Model):
	...
	def _reverse_moves(self, default_values_list=None, cancel=False):
		...
		if reverse_moves.genrated_points > reverse_moves.partner_id.loyalty_pts:
			...
			config = self.env['all.loyalty.setting'].sudo().search([('active','=',True),('issue_date', '<=', today_date ),
									('expiry_date', '>=', today_date ),('loyalty_tier','=',reverse_moves.partner_id.tier_id.id)])
			inv_lines = []
			if config :
	...
	def generate_loyalty_points(self,move):
		...
		today_date = datetime.today().date()
		config = self.env['all.loyalty.setting'].sudo().search([('active','=',True),('issue_date', '<=', today_date ),
									('expiry_date', '>=', today_date ),('loyalty_tier','=',move.partner_id.tier_id.id)])
			...
			if move.is_invoice(include_receipts=True) and move.state == 'posted':
				...
				if currency.is_zero(move.amount_residual):
					...
					if not reconciled_payments or all(payment.is_matched for payment in reconciled_payments):
						new_pmt_state = 'paid'
						self.generate_loyalty_points(move)
					else:
