...
from odoo.addons.website_sale_delivery.models.sale_order import SaleOrder as SaleOrderOrig
    ...
    def _compute_amount_delivery(self):
        for order in self:
            if self.env.user.has_group('account.group_show_line_subtotals_tax_excluded'):
                if order.carrier_id and order.carrier_id.product_id:
                    order.amount_delivery = sum(order.order_line.filtered(lambda x:x.is_delivery and x.product_id == order.carrier_id.product_id).mapped('price_subtotal'))
                else:
                    order.amount_delivery = sum(order.order_line.filtered('is_delivery').mapped('price_subtotal'))
            else:
                if order.carrier_id and order.carrier_id.product_id:
                    order.amount_delivery = sum(order.order_line.filtered(
                        lambda x: x.is_delivery and x.product_id == order.carrier_id.product_id).mapped(
                        'price_total'))
                else:
                    order.amount_delivery = sum(order.order_line.filtered('is_delivery').mapped('price_total'))

    SaleOrderOrig._compute_amount_delivery = _compute_amount_delivery