...
publicWidget.registry.TpFilterAttribute = publicWidget.Widget.extend({
    ...
    _onChangeSearch: function (ev) {
        ...
        if (value) {
            ...
            // Replace logic with :containsTextLike selector in next version so we can avoid lower logic from backend
...
// Move to proper file in next version
...
// Move to proper file
publicWidget.registry.tpAllBrandPageSearch = publicWidget.Widget.extend({
    selector: '.tp-all-brands-page',
    events: {
        'input input.tp-search-brands-input': '_onSearchBrand'
    },
    _onSearchBrand: function (ev) {
        ev.stopPropagation();
        var searchTerm = $(ev.currentTarget).val().trim();
        if (searchTerm) {
            this.$('.tp-brand-block, .tp-brands-group').addClass('d-none');
            var brands = this.$(".tp-brand-name:containsTextLike('" + searchTerm + "')");
            _.each(brands, function (brand) {
                $(brand).parents('.tp-brand-block, .tp-brands-group').removeClass('d-none');
            });
        } else {
            this.$('.tp-brand-block, .tp-brands-group').removeClass('d-none');
        }
    },
});
});
