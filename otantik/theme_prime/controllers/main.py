    ...
    def shop(self, page=0, category=None, search='', ppg=False, **post):
        ...
        if theme_id and theme_id.name.startswith('theme_prime'):
            ...
            if request.website._get_dr_theme_config('json_shop_filters')['show_category_count']:
                ...
                get_category_count = ProductTemplate._get_product_category_count(website_ids=request.website.ids, product_ids=[], domain=domain)
            ...
            if request.website._get_dr_theme_config('json_shop_filters')['show_attrib_count'] or request.website._get_dr_theme_config('json_shop_filters')['hide_attrib_value']:
                ...
                get_attrib_count = ProductTemplate._get_product_attrib_count(website_ids=request.website.ids, product_ids=[], attrib_values=attrib_values, domain=domain)
                response.qcontext.update(
...
class ThemePrimeMainClass(http.Controller):
    ...
    def get_categories_info(self, fields=[], options={}, **kwargs):
        ...
        if options.get('getCount', False):
            get_category_count = request.env['product.template']._get_product_category_count(website_ids=request.website.ids, domain=request.website.website_domain())
