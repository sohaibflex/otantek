...
class SetuInventoryOutOfStockReport(models.TransientModel):
    ...
    def get_products_outofstock_data(self, for_reporting=False):
        """
        :return:
        """
        ...
        fsn_vals_list = {'company_ids': [(4, id) for id in list(company_ids)],
                         'product_ids': [(4, p_id) for p_id in list(products)] if products else [],
                         'product_category_ids': [(4, p_c_id) for p_c_id in list(category_ids)] if category_ids else [],
                         'warehouse_ids': [(4, w_id) for w_id in list(warehouses)] if warehouses else [],
                         'start_date': start_date, 'end_date': end_date}
        fsn_vals = self.env['setu.inventory.fsn.analysis.report'].create(fsn_vals_list)
        fsn_vals.get_inventory_fsn_analysis_report_data()
        # get_products_outofstock_data(company_ids, product_ids, category_ids, warehouse_ids, start_date, end_date, advance_stock_days)
        query = """
                Select * from get_products_outofstock_data('%s','%s','%s','%s','%s','%s', '%s','%d')
            """ % (company_ids, products, category_ids, warehouses, start_date, end_date, self.advance_stock_days,self.id)
        # print(query)
        self._cr.execute(query)
        stock_data = self._cr.dictfetchall()
        if for_reporting:
            self._cr.execute(f"""select * from setu_inventory_outofstock_bi_report where wizard_id={self.id}""")
            stock_data = self._cr.dictfetchall()
            return stock_data
        return stock_data
    ...
    def download_report(self):
        ...
        stock_data = self.get_products_outofstock_data(for_reporting=True)
        ...
        return {
            ...
            'url': '/web/binary/setu_air_download_document?model=setu.inventory.outofstock.report&field=stock_file_data&id=%s&filename=%s'%(self.id, file_name),
            'target': 'self',
        }
...
class SetuInventoryOutOfStockBIReport(models.TransientModel):
    ...
    _description = """Inventory Outofstock Analysis Report"""
