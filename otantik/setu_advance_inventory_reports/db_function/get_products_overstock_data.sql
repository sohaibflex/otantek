...
DROP FUNCTION IF EXISTS public.get_products_overstock_data(integer[], integer[], integer[], integer[], date, date, integer);
DROP FUNCTION IF EXISTS public.get_products_overstock_data(integer[], integer[], integer[], integer[], date, date, integer, integer);
...
BEGIN
    ...
    Select
        final_data.company_id, cmp.name, final_data.product_id, case when prod.default_code is not null then
    ...
    Insert into setu_inventory_overstock_bi_report(product_id, product_category_id, warehouse_id, company_id, sales,ads, qty_available, incoming, outgoing, forecasted_stock,demanded_qty,coverage_days,overstock_qty,overstock_value,wh_overstock_qty_per,wh_overstock_value_per,turnover_ratio,stock_movement,last_purchase_date,last_purchase_qty,last_purchase_price,currency_name,vendor_name,company_name,product_name,category_name,warehouse_name,wizard_id)
        select
         F.product_id, F.product_category_id, F.warehouse_id, F.company_id, F.sales,
         F.ads, F.qty_available, F.incoming, F.outgoing, F.forecasted_stock,F.demanded_qty, F.coverage_days,
         F.overstock_qty,F.overstock_value ,F.warehouse_overstock_qty as wh_overstock_qty_per,F.warehouse_overstock_value as wh_overstock_value_per, F.turnover_ratio,F.stock_movement,F.last_purchase_date ,
         F.last_purchase_qty , F.last_purchase_price , F.currency_name,F.vendor_name,F.company_name,F.product_name,F.category_name,F.warehouse_name,wizard_id
         from(
            with all_data as (
