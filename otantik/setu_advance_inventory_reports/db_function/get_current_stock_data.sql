...
CREATE OR REPLACE FUNCTION public.get_current_stock_data(
    ...
    IN start_date date,
    IN end_date date)
  RETURNS TABLE(company_id integer,product_id integer, product_category_id integer,warehouse_id integer,location_id integer,current_stock double precision) AS
$BODY$
BEGIN
    Return Query
    select
