...
class GiftWrapConfiguration(models.Model):
    ...
    gift_image = fields.Binary(related='product_id.image_128', string="Image", readonly=False,store=True)

    @api.onchange('product_id')
    def _onchange_giftwrap(self):
        if self.product_id:
            self.update({
                'price': self.with_company(self.env.company).lst_price,
            })
...
class SaleInherit(models.Model):
    ...
    def _cart_update(self, product_id=None, line_id=None, add_qty=0, set_qty=0, **kwargs):
        ...
        if quantity <= 0:
            ...
            gift = self.env['giftwrap.configuration'].sudo().search([('product_id', '=', product_id)])
            if gift:
                values['price_unit'] = gift.product_id.with_company(request.env.company).lst_price
