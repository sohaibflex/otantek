    ...
    def _check_available_visitor_data(self):
        ...
        for service in self.filtered(lambda s: s.type == 'ga4'):
            if any(service[track_option] for track_option in [
                'track_ip_address', 'track_user_agent', 'track_email', 'track_phone', 'track_country', 'track_city',
            ]):
                raise ValidationError(_('Only the following data can be sent to GA4: External ID.'))

    def get_item_categories(self, product, property_name: str = 'content_category'):
        self.ensure_one()
        if self.type != 'ga4':
            return super(WebsiteTrackingService, self).get_item_categories(product, property_name)  # flake8: noqa: E501
        res = {}
        if self.category_type == 'product':
            ...
            if category:
                for index, category in enumerate(category.parents_and_self[:5]):
                    res.update({'item_category%s' % ((index + 1) if index > 0 else ''): category.name})  # flake8: noqa: E501
            else:
