...
class Digest(models.Model):
    ...
    def get_account_total_revenue(self, domain):
        ...
        for record in self:
            ...
            query = f"""SELECT -SUM(line.balance)
                ...
                WHERE line.company_id = {company.id} AND date(line.date) > '{start}' AND date(line.date) <= '{end}'
    ...
    def get_total_orders_count(self, domain):
        ...
        for record in self:
            ...
            query = f"""select count(*) from sale_order so where so.company_id ={company.id} 
                AND date(so.date_order) >= '{start}' AND date(so.date_order) <= '{end}' 
    ...
    def get_shipped_orders_count(self, domain):
        ...
        for record in self:
            ...
            query = f"""select count(*) from stock_picking sp
                 ...
                 and date(sp.date_done) >= '{start}' and date(sp.date_done) <= '{end}' {domain}"""
            self._cr.execute(query)
            query_res = self._cr.fetchone()
    ...
    def get_pending_shipment_on_date_count(self, domain):
        ...
        for record in self:
            ...
            query = f"""select count(*) from stock_picking sp
                 ...
                 join stock_location on stock_location.id=sp.location_dest_id and stock_location.usage='customer' 
                 where sp.state != 'cancel' and sp.company_id={company.id} 
                 and date(sp.scheduled_date) >= '{start}' and date(sp.scheduled_date) <= '{end}' {domain}"""
            self._cr.execute(query)
            query_res = self._cr.fetchone()
    ...
    def get_cancel_orders_count(self, domain):
        ...
        for record in self:
            ...
            domain = self._prepare_query_domain(domain, 'so.')
            query = f"""select count(*) from sale_order so where so.company_id ={company.id}
                AND date(so.date_order) >= '{start}' AND date(so.date_order) <= '{end}' and state='cancel' {domain}"""
            self._cr.execute(query)
            query_res = self._cr.fetchone()
    ...
    def get_orders_average(self, domain):
        ...
        for record in self:
            ...
            query = f"""select sum(amount_untaxed) from sale_order so
                        where so.company_id ={company.id} AND date(so.date_order) >= '{start}' AND date(so.date_order) <= '{end}' 
                        and state in ('sale','done') {domain}"""
            self._cr.execute(query)
            query_res = self._cr.fetchone()
            total_sales = 0.0
    ...
    def get_refund_orders_count(self, domain):
        ...
        for record in self:
            ...
            query = f"""select count(*) from account_move am
                where am.company_id = {company.id} AND date(am.invoice_date) > '{start}' 
                AND date(am.invoice_date) <= '{end}' and am.move_type='out_refund' {domain} group by am.id"""
            self._cr.execute(query)
            query_res = self._cr.fetchone()
            total_refund_orders = []
            if query_res != None:
    ...
    def compute_kpis(self, company, user):
        ...
        if self.is_connector_digest:
            ...
            for col_index, (tf_name, tf) in enumerate(self._compute_timeframes(company)):
                digest = self.with_context(start_date=tf[0][0], end_date=tf[0][1]).with_user(user).with_company(
                    company)
                previous_digest = self.with_context(start_date=tf[1][0], end_date=tf[1][1]).with_user(
                    user).with_company(company)
                for index, field_name in enumerate(digest_fields):
                    kpi_values = kpis[index]
                    self.with_context(start_date=tf[0][0],
                                      end_date=tf[0][1])._prepare_domain_based_on_connector()
        ...
        return super(Digest, self).compute_kpis(company, user)
