...
class StockPicking(models.Model):
    ...
    def _get_cover_amount_without_qty_done(self):
        ...
        move_line_ids = picking_move_lines.filtered(lambda ml: float_compare(ml.qty_done, 0.0, precision_rounding=ml.product_uom_id.rounding) > 0 and not ml.result_package_id)

        if not move_line_ids:
            move_line_ids = picking_move_lines.filtered(lambda ml: 
                        float_compare(ml.product_uom_qty, 0.0, precision_rounding=ml.product_uom_id.rounding) > 0 and 
        ...
        if not total_weight:
            total_weight = sum([po.product_uom_qty * po.product_id.weight for po in move_line_ids]) 
    ...
    def action_put_in_pack(self):
        ...
        if res and (type(res) == dict):
            ...
            ctx = {
                'no_description':  not(delivery_type in ['my_dhl','fedex', 'dhl', 'ups', 'auspost', 'canada_post', 'royal_mail']),
                'no_cover_amount': not(delivery_type in ['my_dhl','fedex', 'dhl', 'ups', 'usps', 'auspost', 'canada_post', 'royal_mail']),
                'no_edt_document': not(delivery_type in ['fedex', 'ups']),
            ...
            if carrier_id and carrier_id.delivery_type not in ['base_on_rule', 'fixed']:
                ...
                ctx['default_length'] = self.carrier_id.packaging_id.packaging_length
            ...
            """
            vals = dict(
                delivery_packaging_id=self.carrier_id.packaging_id.id,
                height=self.carrier_id.packaging_id.height,
                width=self.carrier_id.packaging_id.width,
                wkk_length=self.carrier_id.packaging_id.packaging_length,
                cover_amount=cover,
